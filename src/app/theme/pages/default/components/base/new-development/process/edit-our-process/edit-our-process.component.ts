...
@Component({
    selector: 'app-edit-our-process',
    templateUrl: './edit-our-process.component.html',
    encapsulation: ViewEncapsulation.None,
    styles: []
})
export class EditOurProcessComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.getid = this._route.snapshot.paramMap.get('processId');
        ...
        this._brewery.getOurProcessList(this.subdomainID, this.getid).subscribe(
            res => {
                this.processData = res.data[0];
                this.blogLogo = 'http://api.fftb.co.uk/image/ourprocess/' + this.processData.id;
                this.model.process_name = this.processData.process_name;
                this.model.process_title = this.processData.process_title;
                this.model.process_image = this.processData.process_image;
                this.model.show_on_page = this.processData.show_on_page;
                this.model.process_short_des = this.processData.process_short_des;
                $('#my-summernote').summernote('code', this.processData.process_short_des);
    ...
    /**
     * Method to edit ourprocess image
     */
    onSubmit() {
        ...
        formData.append('process_name', this.ourprocessForm.patchValue({ process_name: this.model.process_name }));
        formData.append('process_title', this.ourprocessForm.patchValue({ process_title: this.model.process_title }));
        formData.append('show_on_page', this.ourprocessForm.patchValue({ show_on_page: this.model.show_on_page }));
        formData.append('process_image', this.ourprocessForm.patchValue({ process_image: this.model.process_image }));
        //console.log(this.ourprocessForm.value);
        this._brewery.updateOurProcess(this.ourprocessForm.value, this.getid).subscribe(
            res => {
                if (res['status'] == true) {
