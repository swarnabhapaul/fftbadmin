...
@Component({
    selector: 'app-edit-what-we-learned-so-far',
    templateUrl: './edit-what-we-learned-so-far.component.html',
    styles: []
})
export class EditWhatWeLearnedSoFarComponent implements OnInit {
    ...
    private shortContent: any;
    private longContent: any;
    ...
    ngOnInit() {
        ...
        this._brewery.getWhatWeLearnedDetailsById(this.subdomain, this.getid).subscribe(
            res => {
                this.whatwelearnedData = res.data[0];
                this.empLogo = 'http://api.fftb.co.uk/image/whatwelearned/' + this.whatwelearnedData.id;
                this.model.heading = this.whatwelearnedData.heading;
                this.model.image = this.whatwelearnedData.image;
                this.model.short_desc = this.whatwelearnedData.short_desc;
                $('#my-summernote-short').summernote('code', this.whatwelearnedData.short_desc);
                this.model.long_desc = this.whatwelearnedData.long_desc;
                $('#my-summernote-long').summernote('code', this.whatwelearnedData.long_desc);
                //console.log(this.whatwelearnedData);
            }, err => {
                console.log(err);
    ...
    ngAfterViewInit() {
        this._script.loadScripts('app-edit-what-we-learned-so-far',
    ...
    /**
     * Method to edit whatwelearned image
     */
    onSubmit() {
        ...
        formData.append('heading', this.whatwelearnedForm.patchValue({ heading: this.model.heading }));
        formData.append('image', this.whatwelearnedForm.patchValue({ image: this.model.image }));

        this._brewery.updateWhatWeLearnedDetails(this.whatwelearnedForm.value, this.getid, this.subdomain).subscribe(
            res => {
                if (res['status'] == true) {
