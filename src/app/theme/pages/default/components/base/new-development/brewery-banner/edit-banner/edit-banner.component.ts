...
export class EditBannerComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.getid = this._route.snapshot.paramMap.get('bannerId');
        ...
        this._brewery.getBannerDetails(this.getid).subscribe(
            res => {
                console.log('res', res);
                this.bannerData = res.data[0];
                this.bannerLogo = 'http://api.fftb.co.uk/image/banner/' + this.bannerData.id;
                this.model.banner_type = this.bannerData.banner_type;
                this.model.banner_image = this.bannerData.banner_image;
                this.model.banner_content = this.bannerData.banner_content;
                $('#my-summernote').summernote('code', this.bannerData.banner_content);
    ...
    uploadImg(event) {
        ...
        reader.onload = (e: any) => {
            ...
            this._common.uploadImg(file, this.IMG_DIR).subscribe(data => {
                ...
                this.bannerLogo = e.target.result;
                console.log(this.bannerLogo);
                this.model.banner_image = data.data[0];
    ...
    /**
     * Method to edit banner image
     */
    onSubmit() {
        let formData = new FormData();
        this.bannerContent = $('#my-summernote').val();

        formData.append('banner_content', this.bannerForm.patchValue({ banner_content: this.bannerContent }));
        formData.append('banner_type', this.bannerForm.patchValue({ banner_type: this.model.banner_type }));
        formData.append('banner_image', this.bannerForm.patchValue({ banner_image: this.model.banner_image }));
        console.log(this.bannerForm.value);
        this._brewery.updateBanner(this.bannerForm.value, this.getid).subscribe(
            res => {
                if (res['status'] == true) {
