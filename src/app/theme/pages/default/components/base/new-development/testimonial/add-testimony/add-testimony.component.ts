...
export class AddTestimonyComponent implements OnInit {
    ...
    // readonly MAX_FILE_SIZE_IN_BYTE: number = 1024000;
    ...
    onSubmit() {
        ...
        formData.append('organisation_name', this.testimonialForm.organisation_name);
        formData.append('author', this.testimonialForm.author);
        formData.append('role', this.testimonialForm.role);
        formData.append('comment', this.testimonialForm.patchValue({ comment: this.blogContent }));

        this._brewery.addOurTestimonial(this.testimonialForm.value, this.bname).subscribe(
            res => {
                if (res['status'] == true) {
